...
// -------------------------------------------------------------
// GOOGLE DRIVE INTEGRATION & SETUP MODAL
// -------------------------------------------------------------
function initGoogleAuth() {
  if (!state.clientId) {
    // Check if user has stored client ID or prompt configuration
    return;
  }
...
function openGDriveConfigModal() {
  ...
  if (originEl) {
    originEl.innerText = window.location.origin || 'https://yourwebsite.com';
  }
...
function saveGDriveClientId() {
  ...
  showToast('Google Client ID saved! Click Sign in again');
}
...
function handleGDriveUrlPaste(type) {
  ...
  document.getElementById(`name-${type}`).innerText = `Google Drive File (${fileId.substring(0, 8)}...)`;
...
// -------------------------------------------------------------
// FILE UPLOAD ENGINE: DIRECT STREAM + GOOGLE DRIVE
// -------------------------------------------------------------
async function handleFileUpload(event, type) {
  ...
  if (state.accessToken) {
    try {
      ...
      progressPercent.innerText = '50% (Uploading to your Google Drive...)';
      ...
      if (gData && gData.id) {
        ...
        progressPercent.innerText = '80% (Setting public sharing...)';
  ...
  if (!directPublicUrl) {
    try {
      ...
      progressPercent.innerText = '75% (Connecting Gofile Server...)';
