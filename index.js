...
// PERMANENT FOREVER STORAGE ENGINE (Images Forever via FreeImage + Multi-Files via Gofile/Direct Bridge)
    ...
    try {
      if (type === 'image') {
        ...
        if (resData && resData.image && resData.image.url) {
          directPublicUrl = resData.image.url;
        }
      }

      if (!directPublicUrl) {
        // 2. PERMANENT FILE STORAGE FOR PDF, AUDIO, VIDEO (Gofile 100MB+ Forever)
        try {
          const srvRes = await fetch('https://api.gofile.io/servers');
          const srvData = await srvRes.json();
          const serverName = (srvData && srvData.data && srvData.data.servers && srvData.data.servers[0]) ? srvData.data.servers[0].name : 'store3';

          const gofileForm = new FormData();
          gofileForm.append('file', file);

          const upRes = await fetch(`https://${serverName}.gofile.io/contents/uploadfile`, {
            method: 'POST',
            body: gofileForm
          });
          const upData = await upRes.json();
          if (upData && upData.status === 'ok' && upData.data && upData.data.downloadPage) {
            directPublicUrl = upData.data.downloadPage;
          }
        } catch (gofileErr) {
          console.warn('Gofile upload fallback:', gofileErr);
        }
      }

      if (!directPublicUrl) {
        // 3. Fallback to Direct tmpfiles dl bridge
        ...
        if (tmpData && tmpData.status === 'success' && tmpData.data && tmpData.data.url) {
          directPublicUrl = tmpData.data.url.replace('tmpfiles.org/', 'tmpfiles.org/dl/');
        }
      }
      ...
      setTimeout(() => {
        ...
        updateQrCode();
        showToast(`⚡ ${type.toUpperCase()} Permanent Forever QR Ready!`);
      }, 400);

    } catch (err) {
      console.warn('Upload error, fallback to Data URI:', err);
      progressBox.style.display = 'none';
...
function renderPayloadChip(payload) {
  ...
  if (payload.startsWith('http')) {
    chip.innerHTML = `<i class="fa-solid fa-infinity" style="color: var(--accent-emerald);"></i> ${escapeHtml(payload)}`;
...
function copyDirectLink() {
  ...
  navigator.clipboard.writeText(payload).then(() => {
    showToast('Permanent direct link copied to clipboard!');
  }).catch(() => {
...
function handleScannedResult(rawString) {
  ...
  if (rawString.startsWith('http://') || rawString.startsWith('https://')) {
    ...
    } else if (lower.endsWith('.png') || lower.endsWith('.jpg') || lower.endsWith('.jpeg') || lower.endsWith('.webp') || lower.endsWith('.gif') || lower.includes('iili.io')) {
